...
- **按实际打开的工作区显示：** 能确认根目录的已打开项目都会保留入口；尚无 Codex 会话或删除当前任务后，显示灰色空心圆和“暂无任务”，仍可点击回到窗口。已有会话归到打开的根目录；本地多项目工作区显示一条工作区入口，支持已保存的 `.code-workspace` 和未命名工作区。同一目录在单独窗口与多项目工作区中同时打开时，各自保留入口。
- **自动识别 Git worktree 关系：** 同仓库的工作目录连续显示，仓库名只出现一次。各行以分支短名区分，关联工作树带 `WT` 标记；完整分支名和路径可在悬停提示或任务详情中查看，无需依赖被截断的项目全名。
- **用树形展示分支来源：** Git 创建记录能确认来源且来源分支窗口也已打开时，父分支在前、子分支缩进连线；从子分支继续创建，也能逐级显示。每个工作目录保留独立窗口入口，有任务时显示各自的状态与详情。
...
总目录中混有项目或附件目录时，可从顶部“管理目录”取消勾选，或右键分类选择“排除此目录”，停止读取并隐藏该目录；重新勾选即可恢复，原文件保留。未排除的空分类照常显示。
...
窗口切换要求所选根目录或工作区能够唯一匹配。“暂无任务”仅表示尚无可显示的任务，不代表 Codex 扩展已连接，也不会打开会话预览；欢迎页和无法确认根目录的窗口不生成项目入口。会话预览依赖扩展的内部本地接口；重复窗口、同名歧义、远程任务等边界见[支持范围](docs/USER_GUIDE.md#支持范围)。
