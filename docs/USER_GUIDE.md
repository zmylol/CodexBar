...
当前连接一个总目录，其一级文件夹作为各知识库分类；根目录散放的 Markdown、隐藏文件及符号链接不参与。各库共用后台扫描预算：最多 10,000 篇、每篇 256 KiB；变更预览的正文缓存最多 32 MiB，超限或读取失败会提示。文章列表使用独立索引，正文缓存满后仍可发现新文章；每篇标题最多 1 KiB、摘要最多 4 KiB，较长文本显示省略号，全文仍可在 Obsidian 阅读。收录元数据及正文只在内存中处理，不新增磁盘索引或改写文章；无法精确区分文件写入来自 Codex、Obsidian 还是其他工具。

如果原路径下的知识库文件夹被另一个目录替换，CodexBar 会提示并重新建立基线，清除旧库的审阅记录和已读状态；不会把旧库所有笔记误报为删除。普通文章改名或移动会保留已读状态，无法唯一确认文件身份时则保守地作为新路径处理。
