...
## 协议兼容与生命周期回归（2026-09-12）

仍只接受 `thread-stream-state-changed` v11，没有放宽版本、owner、host 或目标客户端校验。遇到其他版本时，monitor 保存预期版本和收到的版本，并停止当前跟随；预览会明确提示扩展版本暂不兼容。原因跨断线和重试保留，关闭后重新展开仍立即可见。显式刷新可以重新协商，收到可信 owner 的 v11 帧后才清除原因并恢复正文；普通 Hook 不触发持续重试。

自动回归覆盖 v12 拒绝与原因查询、显式重试、普通断线不覆盖原因、v11 恢复；coordinator 的无窗口检查覆盖切换会话时迟到帧、失效 worker 的迟到结果、关闭后的历史回调、断线保留正文，以及超时不覆盖不兼容提示。AppModel 保留 runtime 串行队列，正文相关的目标、generation、worker 与等待任务集中到 `ConversationPreviewCoordinator`。

`Tests/CodexBarCoreTests/Fixtures/runtime-v11-synthetic.json` 是完全合成的匿名协议样本，包含一份 snapshot 和一批 patches，使用虚构 session、owner、cwd 和正文。同一组帧分别进入 runtime 与正文 reducer，检查审批状态和助手正文的解释是否保持一致；该样本没有采集真实用户会话，也不是扩展版本兼容性的真实传输证明。

本轮新增的是合成 router、固定 fixture 与 Model 生命周期回归，没有重新验证真实 VS Code 或 Codex 扩展。上方历史验证表中的版本组合仍是已有记录；升级后需重新执行真实连接、正文更新、历史加载与断线恢复验证，不能把合成回归通过当作新扩展版本已兼容。
