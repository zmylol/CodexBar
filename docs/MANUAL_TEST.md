...
恢复多个 VS Code 窗口后，点击目标项目右侧靶心图标，确认“专注此项目”会恢复并前置目标窗口、最小化其他标准 VS Code 窗口。悬停该图标应显示“专注此项目：最小化其他 VS Code 窗口”；键盘可以聚焦并触发该按钮，VoiceOver 能读出按钮作用与最小化影响，原三点菜单仍可使用。

失败关闭场景：

1. 关闭一个目标窗口，收到系统窗口通知后对应项目和正在显示的详情应自动收起；最小化窗口时项目仍保留。
2. 打开两个标题都匹配同一项目名的窗口，分别尝试普通切换和靶心图标专注，应提示歧义，不得切换或最小化任何窗口。
...
8. 切换到其他应用并点击对应项目，确认原窗口被准确恢复，其他 VS Code 窗口保持原有最小化状态；点击项目右侧靶心图标，确认专注时才最小化其他标准 VS Code 窗口。
