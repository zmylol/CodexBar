...
2. 工作中的项目显示“执行中”。
3. 持续悬停有计划的项目，确认显示已完成勾选、当前进度环、待办空心圆和“第 N / M 步”；计划推进时无需移开鼠标即可更新，超过五步时只显示围绕当前步骤的五行。
4. 悬停没有计划的项目，确认仍最多动态显示三个“读取 / 搜索 / 修改 / 测试 / 命令”节点；两种视图都不得显示原始命令、补丁、搜索词、计划解释或工具输出。
...
10. 不使用鼠标，以键盘聚焦任务行，确认出现同一详情；开启 VoiceOver 后应朗读 N/M、已完成数、当前/等待/完成态及最多五个可见步骤状态。
11. 分别在浅色、深色、Increase Contrast、Reduce Transparency 和较大文字设置下检查计划；标题、进度文字与三种状态图标应清晰，截断标题悬停时应显示完整 tooltip。
