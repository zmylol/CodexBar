...
public enum CodexBarPanelLayout {
    ...
    public static let detailHeight: CGFloat = 132
    ...
    public static func detailFrame(
        ...
        let proposedY = rowCenterY - detailHeight / 2
        let maximumY = max(visibleFrame.minY, visibleFrame.maxY - detailHeight)
        let y = min(max(proposedY, visibleFrame.minY), maximumY)

        return CGRect(x: x, y: y, width: detailWidth, height: detailHeight)
    }
}
