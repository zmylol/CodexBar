...
自动连接或手动选择后，CodexBar 在后台扫描该总目录内的普通 Markdown 文件，跳过隐藏目录、隐藏文件和符号链接；首次读取建立基线，递归文件系统事件触发后续检查，没有定时轮询。总目录下的一级文件夹显示在独立知识库面板中，各分类共享扫描、基线与读取预算；文章的 type、collected、title、正文一级标题及摘要首段在安全读取后提取到内存，用于按北京时间显示当日收录文章；所选知识库仅保留在面板内存中。每 30 秒及打开面板时重新检查缓存日期，不为日期切换扫描文件夹。正文与文件元数据缓存有 10,000 篇笔记、单篇 256 KiB、总正文 32 MiB 的读取限制；单条差异最多 32 KiB，回看记录另有 512 条、4 MiB 上限。超限或读取失败会显示提示。今日清单只包含带有效收录日期的文章，不包含旧文修改；文件写入来源不作推断，也不读取 Codex 桌面端的会话数据。
