...
打开会话预览时，CodexBar 会在内存中保留该会话的界面状态，并显示用户消息、助手正文、命令及工具结果。这里显示原始可见内容，不使用 Hook 摘要的脱敏规则；正文可能包含用户在会话中提供的敏感信息。只有当前展开的一个会话会保留预览状态，关闭、切换、删除任务或退出应用时释放，不写入 `tasks.json`、事件归档或日志。断线时可以继续显示本次已接收内容，并明确提示连接状态。预览状态设有 32 MiB 内容预算及结构上限，超限时提示无法读取，不静默截短工具输出。点击“加载较早内容”会向已有会话 owner 请求补齐历史；此操作不启动、恢复或执行 turn。未展开会话的正文在状态处理后释放。
